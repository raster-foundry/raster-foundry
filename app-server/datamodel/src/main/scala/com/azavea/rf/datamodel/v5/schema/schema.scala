package com.azavea.rf.datamodel.v5.schema
...
trait tables {
   ...
   *  @param authId Database column auth_id SqlType(text), Default(None) */
  case class UsersRow(id: java.util.UUID, organizationId: java.util.UUID, authId: Option[String] = None)
  /** GetResult implicit for fetching UsersRow objects using plain SQL queries */
  implicit def GetResultUsersRow(implicit e0: GR[java.util.UUID], e1: GR[Option[String]]): GR[UsersRow] = GR{
    prs => import prs._
  ...
  class Users(_tableTag: Tag) extends Table[UsersRow](_tableTag, "users") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(organizationId), authId).shaped.<>({r=>import r._; _1.map(_=> UsersRow.tupled((_1.get, _2.get, _3)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))

    /** Database column id SqlType(uuid), PrimaryKey */
    val id: Rep[java.util.UUID] = column[java.util.UUID]("id", O.PrimaryKey)
    /** Database column organization_id SqlType(uuid) */
    val organizationId: Rep[java.util.UUID] = column[java.util.UUID]("organization_id")
    /** Database column auth_id SqlType(text), Default(None) */
    val authId: Rep[Option[String]] = column[Option[String]]("auth_id", O.Default(None))
