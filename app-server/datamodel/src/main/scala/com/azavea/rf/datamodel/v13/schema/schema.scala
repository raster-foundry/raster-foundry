package com.azavea.rf.datamodel.v13.schema
...
trait tables {
  ...
  lazy val schema: profile.SchemaDescription = Array(Buckets.schema, Footprints.schema, Organizations.schema, Scenes.schema, ScenesToBuckets.schema, Thumbnails.schema, Users.schema, UsersToOrganizations.schema).reduceLeft(_ ++ _)
  ...
  case class FootprintsRow(id: java.util.UUID, organizationId: java.util.UUID, createdAt: java.sql.Timestamp, modifiedAt: java.sql.Timestamp, multipolygon: Projected[Geometry])
  ...
  implicit def GetResultFootprintsRow(implicit e0: GR[java.util.UUID], e1: GR[java.sql.Timestamp], e2: GR[Projected[Geometry]]): GR[FootprintsRow] = GR{
    prs => import prs._
    FootprintsRow.tupled((<<[java.util.UUID], <<[java.util.UUID], <<[java.sql.Timestamp], <<[java.sql.Timestamp], <<[Projected[Geometry]]))
  }
  ...
  class Footprints(_tableTag: Tag) extends Table[FootprintsRow](_tableTag, "footprints") {
    def * = (id, organizationId, createdAt, modifiedAt, multipolygon) <> (FootprintsRow.tupled, FootprintsRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(organizationId), Rep.Some(createdAt), Rep.Some(modifiedAt), Rep.Some(multipolygon)).shaped.<>({r=>import r._; _1.map(_=> FootprintsRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
