...
trait ExtendedPostgresDriver extends ExPostgresDriver
  ...
  object RFDatabaseJsonProtocol extends DefaultJsonProtocol {
  ...
  object RFAPI extends API with ArrayImplicits
      with JsonImplicits {

    import RFDatabaseJsonProtocol._
    ...
    implicit val jobStatusTypeMapper = createEnumJdbcType[JobStatus]("JobStatus", _.toString,
      JobStatus.values.get(_).get, quoteName = false)
    implicit val jobStatusTypeListMapper = createEnumListJdbcType[JobStatus]("JobStatus",
      _.toString, JobStatus.values.get(_).get, quoteName = false)
    ...
    implicit val visibilityTypeMapper = createEnumJdbcType[Visibility]("Visibility", _.toString,
      Visibility.values.get(_).get, quoteName = false)
    implicit val visibilityTypeListMapper = createEnumListJdbcType[Visibility]("Visibility",
      _.toString, Visibility.values.get(_).get, quoteName = false)
