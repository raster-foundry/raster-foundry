...
class DataVizHistogramController {
    constructor($rootScope, $scope, $log, $q, $element, $window, graphService) {
        'ngInject';
    ...
    visualizeHistogram() {
        this.initGraph();
        this.getGraph().then(graph => {
            const plot = this.createPlotFromHistogram(this.histogramData, this.histogramBounds);
    ...
    createPlotFromHistogram(histogram, bounds) {
        ...
        if (buckets.length < 100) {
            ...
            range(0, 100).forEach(mult => {
                const key =
                    Math.round((mult * diff + this.options.dataRange.min) / this.precision) *
                    this.precision;
...
export default angular
    ...
    .component('rfDataVizHistogram', component).name;
