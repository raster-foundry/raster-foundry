...
export default app => {
    class PlanetRepository {
                ...
                const search = () => {
                    return this.$q((resolve, reject) => {
                                ...
                                .then(
                                    res => {
                                        if (res.status === 200) {
                                            ...
                                            reject(
                                                `Unexpected planet API response code: ${res.status}`
                                            );
                                        }
                                    },
                                    err => {
                        ...
                        const continueSearch = () => {
                            this.planetLabsService
                                .getFilteredScenesNextPage(this.planetToken, nextLink)
                                .then(
                                    res => {
                                        if (res.status === 200) {
                                            /* eslint-disable-next-line max-len */
        ...
        getThumbnail(scene, trim = false) {
            return this.$q((resolve, reject) => {
                this.planetLabsService
                    .getThumbnail(this.planetToken, scene.thumbnails[0].url, this.thumbnailSize)
                    .then(
                        response => {
                            let thumbnail = `data:image/png;base64,${response}`;
                            if (trim) {
                                this.trimThumbnail(thumbnail).then(
                                    trimmed => {
                                        resolve(trimmed);
                                    },
                                    err => {
        ...
        trimThumbnail(thumbnail) {
            ...
            return this.$q((resolve, reject) => {
                ...
                image.onload = () => {
                    ...
                    while (bound.top < bound.bottom && rowBlank(pixels, width, bound.top)) {
                        bound.top = bound.top + 1;
                    }
                    while (
                        bound.bottom - 1 > bound.top &&
                        rowBlank(pixels, width, bound.bottom - 1)
                    ) {
        ...
        addToProject(projectId, scenes) {
            ...
            const getDatasourceId = datasource =>
                _.first(_.filter(this.sources, s => s.id === datasource)).uuid;
            return this.$q((resolve, reject) => {
