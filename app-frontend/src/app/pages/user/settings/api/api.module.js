...
class ApiTokensController {
    ...
    $onInit() {
        ...
        if (this.$stateParams.code) {
            this.tokenService.createApiToken(this.$stateParams.code).then(
                authResult => {
                    this.modalService
                        .open({
    ...
    createToken(name) {
        this.authService.createRefreshToken(name).then(
            authResult => {
                this.modalService
                    .open({
                        component: 'rfRefreshTokenModal',
    ...
    deleteToken(token) {
        ...
        const modal = this.modalService.open({
            ...
            resolve: {
                ...
                subtitle: () =>
                    'Deleting this refresh token will make ' + 'any further requests with it fail',
                content: () =>
                    '<h2>Do you wish to continue?</h2>' + '<p>This is a permanent action.</p>',
