...
class Authenticators(val xa: Transactor[IO])
  ...
  val tokensAuthenticator =
    Kleisli[OptionT[IO, ?], Request[IO], UserWithPlatform](
      {
        ...
        case _ =>
          OptionT.none[IO, UserWithPlatform]
      }
    )
  ...
  ): List[OptionT[IO, UserWithPlatform]] = {
    List(
