...
class AnalysisRoutes extends Authentication
  with LazyLogging
  ...
  def preflight(analysisId: UUID, user: User) = {
    traceName("analysis-preflight") {
      parameter(
        'node.?,
        'voidCache.as[Boolean].?(false)
      ) { (node, void) =>
        val nodeId = node.map(UUID.fromString(_))
        onSuccess(LayerCache.analysisEvalRequirements(analysisId, nodeId, user, void).value) { _ =>
          complete {
  ...
  /** Endpoint used to verify that a [[com.azavea.rf.datamodel.Analysis]] is sufficient to
    *  evaluate the [[com.azavea.rf.datamodel.Template]] to which it refers
    *  TODO Is this endpoint neccessary any longer?
    */
  def validate(analysisId: UUID, user: User) = {
    traceName("analysis-validate") {
      pathPrefix("validate") {
        complete {
          for {
            (_, ast) <- LayerCache.analysisEvalRequirements(analysisId, None, user)
  ...
  /** Endpoint used to get a [[com.azavea.rf.datamodel.Analysis]] histogram */
  def histogram(analysisId: UUID, user: User) = {
    traceName("analysis-histogram") {
      pathPrefix("histogram") {
        parameter(
          ...
          complete {
            val nodeId = node.map(UUID.fromString(_))
            LayerCache.modelLayerGlobalHistogram(analysisId, nodeId, user, void).value
          }
        }
      }
    }
  ...
  /** Endpoint used to get a [[Analysis]] statistics */
  def statistics(analysisId: UUID, user: User) = {
    traceName("analysis-statistics") {
      pathPrefix("statistics") {
        parameter(
          ...
          complete {
            val nodeId = node.map(UUID.fromString(_))
            LayerCache.modelLayerGlobalHistogram(analysisId, nodeId, user, void).mapFilter(_.statistics).value
          }
        }
      }
    }
  ...
  /** The central endpoint for ModelLab; serves TMS tiles given a [[Analysis]] specification */
  def tms(
    analysisId: UUID, user: User
  ): Route =
    (handleExceptions(interpreterExceptionHandler) & handleExceptions(circeDecodingError)) {
      traceName("analysis-tms") {
        pathPrefix(IntNumber / IntNumber / IntNumber) { (z, x, y) =>
          parameter(
            ...
            val components = for {
              (lastUpdateTime, ast) <- LayerCache.analysisEvalRequirements(analysisId, nodeId, user)
              (expression, metadata) <- OptionT.pure[Future](ast.asMaml)
              cMap  <- LayerCache.analysisColorMap(analysisId, nodeId, user, colorRamp, colorRampName)
              ...
              components.value.flatMap({ data =>
                val result: Future[Option[Png]] = data match {
                  case Some((expression, metadata, cMap, updateTime)) =>
                    val cacheKey = s"analysis-tms-${analysisId}-${nodeId}-$z-$x-$y-${updateTime.getTime}"
