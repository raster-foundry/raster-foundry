package com.azavea.rf.tile.analysis
...
case class AnalysisParams(
  bands: Array[Int],
  breaks: Array[Double],
  ramp: ColorRamp
)

object AnalysisParams {
  def analysisParams(defaultColorRamp: Option[ColorRamp] = None, defaultBreaks: Option[Array[Double]] = None): Directive1[AnalysisParams] =
    (bandsParam('bands) &  colorBreaksParam('breaks, defaultBreaks) & colorRampParam('ramp, defaultColorRamp)).as(AnalysisParams.apply _)
