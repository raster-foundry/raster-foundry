...
final case class UpdateExportStatus(
...
)(implicit xa: Transactor[IO]) extends Job {
  ...
  def run(): Unit = {
    ...
    exportStatus match {
      ...
      case _ =>
        logger.info(s"Export ${exportId} has not yet completed: ${exportStatus}")
    }
  }
  ...
  def updateExportStatus: ConnectionIO[Unit] = for {
    ...
    _ <- logger.info(s"Setting export status to ${copied.exportStatus}").pure[ConnectionIO]
...
object UpdateExportStatus {
  ...
  def main(args: Array[String]): Unit = {
    val job = args.toList match {
      case List(exportId, exportStatus) => UpdateExportStatus(
        UUID.fromString(exportId), ExportStatus.fromString(exportStatus)
      )
