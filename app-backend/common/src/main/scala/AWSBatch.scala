...
trait AWSBatch extends RollbarNotifier with LazyLogging {
  ...
  def submitJobRequest(jobDefinition: String,
    ...
    logger.trace(s"Using ${awsbatchConfig.environment} in AWS Batch")
    ...
    if (runBatch) {
      logger.trace(s"Trying to submit job: ${jobName}")
      try {
        val submitJobResult = batchClient.submitJob(jobRequest)
        logger.trace(s"Submit Job Result: ${submitJobResult}")
      } catch {
        case e: Exception =>
          logger.error(
