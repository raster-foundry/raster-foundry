  ...
  def createWorkspace: Route = authenticate { user =>
    entity(as[Workspace.Create]) { newWorkspace =>
      authorize(user.isInRootOrSameOrganizationAs(newWorkspace)) {
        onComplete(WorkspaceDao.insert(newWorkspace, user).transact(xa).unsafeToFuture) {
          case Success(workspace) => complete((StatusCodes.Created, workspace))
          case Failure(error) =>
            complete(StatusCodes.ClientError(400)("Error creating new workspace", error.getMessage))
        }
      }
    }
  }
    ...
    entity(as[Workspace.Update]) { workspaceUpdate =>
      authorize(user.isInRootOrSameOrganizationAs(workspaceUpdate)) {
        onComplete(
          ...
          case Success(x) => completeSingleOrNotFound(x)
