      ...
      forAll {
        (
        ...
        ) =>
          {
            ...
            assert(
              insertedAoi.shape == insertedShape.id,
            ...
            assert(
              insertedAoi.startTime == aoi.startTime,
              "Start time should round trip with equality"
            )
            assert(
              insertedAoi.approvalRequired == aoi.approvalRequired,
        ...
        ) =>
          {
            val aoiInsertWithOrgUserProjectIO = for {
              ...
              updatedRows <- AoiDao.updateAOI(
                fixupAoiCreate(dbUser, dbProject, aoiUpdate, newShape)
                  .copy(id = dbAoi.id)
              )
            ...
            assert(
              updatedAoi.shape == updatedShape.id,
            ...
            assert(
              updatedAoi.approvalRequired == aoiUpdate.approvalRequired,
              "Approval should be updated"
            )
            assert(
              updatedAoi.startTime == aoiUpdate.startTime,
        ...
        (
        ...
        ) =>
          {
            val aoisInsertWithProjectUserIO = for {
              (dbUser, _, _, dbProject1) <- insertUserOrgPlatProject(
                user,
                org,
                platform,
                project1
              ...
              dbAois1 <- aois1.traverse { (aoi: AOI.Create) =>
                {
                  AoiDao.createAOI(
                    fixupAoiCreate(dbUser, dbProject1, aoi, dbShape),
                    dbUser
                  )
                }
              }
              _ <- aois2.traverse { (aoi: AOI.Create) =>
                {
                  AoiDao.createAOI(
                    fixupAoiCreate(dbUser, dbProject2, aoi, dbShape),
                    dbUser
                  )
                }
              }
        ...
        ) =>
          {
            val aoisInsertAndListIO = for {
              ...
              dbProject1 <- ProjectDao.insertProject(
                fixupProjectCreate(dbUser1, project1)
                  .copy(visibility = Visibility.Private),
                dbUser1
              )
              dbProject2 <- ProjectDao.insertProject(
                fixupProjectCreate(dbUser2, project2)
                  .copy(visibility = Visibility.Private),
                dbUser2
              )
              ...
              dbAois1 <- aois1 traverse { (aoi: AOI.Create) =>
                {
                  AoiDao.createAOI(
                    fixupAoiCreate(dbUser1, dbProject1, aoi, dbShape),
                    dbUser1
                  )
                }
              }
              _ <- aois2 traverse { (aoi: AOI.Create) =>
                {
                  AoiDao.createAOI(
                    fixupAoiCreate(dbUser2, dbProject2, aoi, dbShape),
                    dbUser2
                  )
                }
              }
              listedAois <- AoiDao.listAuthorizedAois(
                dbUser1,
                AoiQueryParameters(),
                page
              )
