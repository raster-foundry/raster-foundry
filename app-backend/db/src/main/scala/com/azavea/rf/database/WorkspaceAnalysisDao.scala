...
object WorkspaceAnalysisDao extends Dao[WorkspaceAnalysis] with LazyLogging {
  ...
  def getWorkspaceAnalyses(workspace: Workspace, user: User): ConnectionIO[List[Analysis]] = {
    query.filter(fr"workspace_id = ${workspace.id}")
      ...
      .flatMap(
        workspaceAnalyses =>
        workspaceAnalyses.map(_.analysisId).toNel match {
          case Some (ids) =>
          ...
          case _ => List.empty[Analysis].pure[ConnectionIO]
        }
      )
  }
}
