...
case class PlainGroupInvitation(
  ...
  def build: ConnectionIO[EmailData] = {
    ...
    for {
      ...
      |
      | To approve or reject this request, visit http://${platformHost}/user/me/${groupType.toString.toLowerCase}s
      |
...
<html>
  <p>${subjectEmail} has invited you to join the ${groupType.toString.toLowerCase} ${groupName}.
     ...
     <a href="http://${platformHost}/user/me/${groupType.toString.toLowerCase}s">
