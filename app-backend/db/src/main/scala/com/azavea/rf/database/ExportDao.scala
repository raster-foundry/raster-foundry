...
object ExportDao extends Dao[Export] {
  ...
  def getExportStyle(export: Export, exportOptions: ExportOptions, user: User)(implicit xa: Transactor[IO]):
      ConnectionIO[Either[SimpleInput, ASTInput]] = {
    (export.projectId, export.analysisId) match {
      // Exporting an analysis
      case (None, Some(analysisId)) => astInput(analysisId, user).map(Right(_))
      // Exporting a project
