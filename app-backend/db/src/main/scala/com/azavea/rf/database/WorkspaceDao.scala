...
object WorkspaceDao extends Dao[Workspace] {
  ...
  def delete(workspaceId: UUID, user: User): ConnectionIO[Int] = {
    query.filter(workspaceId).ownerFilter(user).delete
  }
  ...
  def addAnalysis(workspaceId: UUID, analysisCreate: Analysis.Create, user: User): ConnectionIO[Option[Analysis]] = {
    WorkspaceDao.query
      ...
      .flatMap {
        case Some(workspace) =>
          AnalysisDao.insertAnalysis(analysisCreate, user).flatMap { analysis =>
            WorkspaceAnalysisDao.insert(workspace, analysis).map(_ => Some(analysis))
          }
        case None =>
