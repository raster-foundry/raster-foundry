  ...
  implicit val annotationProjectQueryParametersFilter
    ...
    Filterable[Any, AnnotationProjectQueryParameters] {
      params: AnnotationProjectQueryParameters =>
        val taskStatusF =
          params.projectFilterParams.taskStatusesInclude.toList map { status =>
            val statusString = status.toString
            statusString.size match {
              case 0 => None
              case _ => Some(fr"(ts.summary ->> ${statusString}) > '0'")
            }
          }
          ...
          List(
            ...
            Some(
              Fragment.const("(") ++ Fragments
                .orOpt(taskStatusF: _*) ++ Fragment.const(")")
            )
          )
    }
}
